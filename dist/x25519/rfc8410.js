 ...
 */
function readDerTag(buf, offset, expectedTag) {
    if (offset >= buf.length) {
        throw new Error(`DER decode: expected tag 0x${expectedTag.toString(16).padStart(2, "0")} at offset ${offset}, got end of buffer`);
    }
    ...
    if (tag !== expectedTag) {
        throw new Error(`DER decode: expected tag 0x${expectedTag.toString(16).padStart(2, "0")} at offset ${offset}, got 0x${tag.toString(16).padStart(2, "0")}`);
    }
...
function expectBytesEqual(actual, expected, label) {
    for (let i = 0; i < actual.length; i++) {
        ...
        if (actualByte !== expectedByte) {
            throw new Error(`${label}: byte ${i} mismatch — expected 0x${expectedByte.toString(16).padStart(2, "0")}, got 0x${actualByte.toString(16).padStart(2, "0")}`);
        }
    }
}
// ---------------------------------------------------------------------------
 ...
 */
export function rawPrivateToPkcs8(raw) {
    if (raw.length !== RAW_KEY_LENGTH) {
        throw new Error(`rawPrivateToPkcs8: private scalar must be ${RAW_KEY_LENGTH} bytes, got ${raw.length}`);
    }
 ...
 * @throws  If `raw` is not exactly 32 bytes.
 */
export function rawPublicToSpki(raw) {
    if (raw.length !== RAW_KEY_LENGTH) {
        throw new Error(`rawPublicToSpki: public coordinate must be ${RAW_KEY_LENGTH} bytes, got ${raw.length}`);
    }
 ...
 */
export function pkcs8ToRaw(der) {
    if (der.length !== PKCS8_DER_LENGTH) {
        throw new Error(`pkcs8ToRaw: PKCS#8 DER must be ${PKCS8_DER_LENGTH} bytes, got ${der.length}`);
    }
    ...
    if (outer.length !== der.length - 2) {
        ...
        throw new Error(`pkcs8ToRaw: outer SEQUENCE length ${outer.length} does not match DER size ${der.length}`);
    }
    // 2. version INTEGER = 0.
    ...
    if (version.length !== 1 || versionByte !== 0x00) {
        throw new Error(`pkcs8ToRaw: expected version INTEGER 0 (1 byte), got length ${version.length}`);
    }
    ...
    if (algorithm.length !== 5) {
        throw new Error(`pkcs8ToRaw: algorithm SEQUENCE must be 5 bytes, got ${algorithm.length}`);
    }
    const oid = readDerTag(der, algorithm.offset, TAG_OID);
    if (oid.length !== 3) {
        throw new Error(`pkcs8ToRaw: OID must be 3 bytes, got ${oid.length}`);
    }
    expectBytesEqual(der.subarray(oid.offset, oid.offset + oid.length), X25519_OID_VALUE, "pkcs8ToRaw: algorithm OID");
    ...
    if (outerOctet.length !== RAW_KEY_LENGTH + 2) {
        ...
        throw new Error(`pkcs8ToRaw: outer OCTET STRING must be ${RAW_KEY_LENGTH + 2} bytes, got ${outerOctet.length}`);
    }
    ...
    if (innerOctet.length !== RAW_KEY_LENGTH) {
        throw new Error(`pkcs8ToRaw: inner OCTET STRING must be ${RAW_KEY_LENGTH} bytes, got ${innerOctet.length}`);
    }
 ...
 */
export function spkiToRaw(der) {
    if (der.length !== SPKI_DER_LENGTH) {
        throw new Error(`spkiToRaw: SPKI DER must be ${SPKI_DER_LENGTH} bytes, got ${der.length}`);
    }
    // 1. Outer SEQUENCE (the whole container).
    const outer = readDerTag(der, 0, TAG_SEQUENCE);
    if (outer.length !== der.length - 2) {
        throw new Error(`spkiToRaw: outer SEQUENCE length ${outer.length} does not match DER size ${der.length}`);
    }
    // 2. algorithm SEQUENCE { OID }.
    const algorithm = readDerTag(der, outer.offset, TAG_SEQUENCE);
    if (algorithm.length !== 5) {
        throw new Error(`spkiToRaw: algorithm SEQUENCE must be 5 bytes, got ${algorithm.length}`);
    }
    const oid = readDerTag(der, algorithm.offset, TAG_OID);
    if (oid.length !== 3) {
        throw new Error(`spkiToRaw: OID must be 3 bytes, got ${oid.length}`);
    }
    expectBytesEqual(der.subarray(oid.offset, oid.offset + oid.length), X25519_OID_VALUE, "spkiToRaw: algorithm OID");
    ...
    if (bitString.length !== RAW_KEY_LENGTH + 1) {
        ...
        throw new Error(`spkiToRaw: BIT STRING must be ${RAW_KEY_LENGTH + 1} bytes, got ${bitString.length}`);
    }
    ...
    if (unusedBits !== 0x00) {
        throw new Error(`spkiToRaw: BIT STRING must have 0 unused bits, got ${unusedBits}`);
    }
...
//# sourceMappingURL=rfc8410.js.map